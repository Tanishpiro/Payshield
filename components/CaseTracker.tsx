...
type Progress={caseNumber:string;status:string;createdAt:string;updatedAt:string;outcome:string|null;timeline:{at:string;label:string;outcome?:string}[]};
...
export default function CaseTracker(){
 ...
 {result&&<div className="ps-case-result" aria-live="polite"><span className="ps-tag">{result.status}</span><h3>{result.caseNumber}</h3>{result.outcome&&<p>{result.outcome}</p>}<p className="ps-muted">Last update: {date(result.updatedAt)}</p><ol>{result.timeline.map((event,i)=><li key={i}><strong>{event.label}</strong><time dateTime={event.at}>{date(event.at)}</time>{event.outcome&&<p>{event.outcome}</p>}</li>)}</ol><p className="ps-muted">Synthetic case progress, not an official police investigation. Refresh to check for new updates.</p></div>}
 </div>}</section>;
}
