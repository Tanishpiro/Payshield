...
const shared=transition(r,'note',{version:0,text:'We are reviewing your submitted evidence.',senderVisible:true},'PRIVATE investigator');assert.equal(publicProgress(shared).timeline.at(-1).note,'We are reviewing your submitted evidence.');assert.ok(!JSON.stringify(publicProgress(shared)).includes('PRIVATE investigator'));
