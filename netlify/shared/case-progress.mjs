// Public case-number lookup: never return stored free text or private evidence.
const findings = {
...
export function publicProgress(record) {
 const events=(record.timeline||[]).flatMap(event=>{
  ...
  if(!label)return [];
  return [{at:event.at,label,...(event.kind==='close'&&findings[event.outcome]?{outcome:findings[event.outcome]}:{})}];
 });
