...
export default async (req) => {
 ...
 try {
  if(req.method==='GET') {
   ...
   const record=await store.get(id,{type:'json'});if(!record)return reply({error:'Case not found. Check the case number.'},404);
   return reply(record);
  }
  ...
  const store=getStore({name:'payshield-demo-cases',consistency:'strong'});
  await store.setJSON(record.summary.caseNumber,record,{onlyIfNew:true});
